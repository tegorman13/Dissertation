...
[![](md_extract/assets/fig-toy-model1-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-toy-model1-1.png)
...
[![](md_extract/assets/methodsFig1.png)](http://localhost:4205/Manuscript/output/Assets/methodsFig1.png)
...
[![](md_extract/assets/fig-IGAS-Training1-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-IGAS-Training1-1.png)
...
[![](md_extract/assets/fig-IGAS-Testing1-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-IGAS-Testing1-1.png)
...
[![](md_extract/assets/fig-e2train-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-e2train-1.png)
...
[![](md_extract/assets/fig-e2testa-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-e2testa-1.png)
...
[![](md_extract/assets/fig-e2test1-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-e2test1-1.png)
...
[![](md_extract/assets/fig-taskSpace1-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-taskSpace1-1.png)
...
[![](md_extract/assets/fig-Toy-Model2-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-Toy-Model2-1.png)
...
Descriptive summaries testing deviation data are provided in Table 4 and Figure 11. To model differences in accuracy between groups, we used Bayesian mixed effects regression models to the trial level data from the testing phase. The primary model predicted the absolute deviation from the target velocity band (dist) as a function of training condition (condit), target velocity band (band), and their interaction, with random intercepts and slopes for each participant (id).
...
[![](md_extract/assets/fig-e1-test-dev-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-e1-test-dev-1.png)
...
[![](md_extract/assets/fig-e1-test-vx-1-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-e1-test-vx-1.png)
...
See Table 7 for the full model results. The estimated coefficient for training condition (\\(B\\) = 164.05, 95% CrI \[45.5, 278.85\]) suggests that the varied group tends to produce harder throws than the constant group, but is not in and of itself useful for assessing discrimination. Most relevant to the issue of discrimination is the slope on Velocity Band (\\(B\\) = 0.71, 95% CrI \[0.62, 0.8\]). Although the median slope does fall underneath the ideal of value of 1, the fact that the 95% credible interval does not contain 0 provides strong evidence that participants exhibited some discrimination between bands. The estimate for the interaction between slope and condition (\\(B\\) = -0.14, 95% CrI \[-0.26, -0.01\]), suggests that the discrimination was somewhat modulated by training condition, with the varied participants showing less senitivity between vands than the constant condition. This difference is depicted visually in Figure 13.@tbl-e1-slope-quartile shows the average slope coefficients for varied and constant participants separately for each quartile. The constant participant participants appear to have larger slopes across quartiles, but the difference between conditions may be less pronounced for the top quartiles of subjects who show the strongest discrimination. Figure Figure 14 shows the distributions of slope values for each participant, and the compares the probability density of slope coefficients between training conditions. Figure 15
...
[![](md_extract/assets/fig-e1-bmm-vx-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-e1-bmm-vx-1.png)

(a) Model fit to all 6 bands

[![](md_extract/assets/fig-e1-bmm-vx-2.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-e1-bmm-vx-2.png)
...
| --- | --- | --- | --- | --- | --- |
| Constant | \-0.114 | 0.479 | 0.689 | 0.933 | 1.39 |
| Varied | \-0.208 | 0.270 | 0.586 | 0.899 | 1.30 |

[![](md_extract/assets/fig-e1-bmm-bx2-1-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-e1-bmm-bx2-1.png)

(a) Slope estimates by participant - ordered from lowest to highest within each condition.

[![](md_extract/assets/fig-e1-bmm-bx2-2-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-e1-bmm-bx2-2.png)

(b) Destiny of slope coefficients by training group

Figure 14: Slope distributions between condition

[![](md_extract/assets/fig-e1-indv-slopes-1-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-e1-indv-slopes-1.png)

(a) subset with largest slopes

[![](md_extract/assets/fig-e1-indv-slopes-2-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-e1-indv-slopes-2.png)
...
Descriptive summaries testing deviation data are provided in Table 9 and Figure 17. To model differences in accuracy between groups, we used Bayesian mixed effects regression models to the trial level data from the testing phase. The primary model predicted the absolute deviation from the target velocity band (dist) as a function of training condition (condit), target velocity band (band), and their interaction, with random intercepts and slopes for each participant (id).
...
[![](md_extract/assets/fig-e2-test-dev-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-e2-test-dev-1.png)
...
Descriptive summaries testing deviation data are provided in Table 11 and Figure 18. To model differences in accuracy between groups, we fit Bayesian mixed effects regression models to the trial level data from the testing phase. The primary model predicted the absolute deviation from the target velocity band (dist) as a function of training condition (condit), target velocity band (band), and their interaction, with random intercepts and slopes for each participant (id).
...
[![](md_extract/assets/fig-e3-test-dev-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-e3-test-dev-1.png)
...
[![](md_extract/assets/fig-e3-bmm-dist-1.png)](http://localhost:4205/Manuscript/output/manuscript_files/figure-html/fig-e3-bmm-dist-1.png)
...
See Table 14 for the full model results.
...
Following the procedure used by M. Mcdaniel et al. (2009), we will assess the ability of both ALM and EXAM to account for the empirical data when fitting the models to 1) only the training data, and 2) both training and testing data. Models will be fit directly to the trial by trial data of each individual participants, both by minimizing the root-mean squared deviation (RMSE), and by maximizing log likelihood. Because ALM has been shown to do poorly at accounting for human patterns extrapolation (DeLosh et al., 1997), we will also fit the extended EXAM version of the model, which operates identically to ALM during training, but includes a linear extrapolation mechanism for generating novel responses during testing.